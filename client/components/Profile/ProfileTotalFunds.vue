...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      prices: 'market2/getMarketHistory'
    }),
    baseAsset() {
    ...
    totalFunds() {
      if (!this.prices(this.baseId)[this.fiatId]) return 0;

      const totalBaseValue = Object.keys(this.balances).reduce((result, id) => {
        if (!this.prices(this.baseId)[id]) return result;
        if (id === this.baseId) return result + this.balances[id].balance;
        return result + (this.balances[id].balance * this.prices(this.baseId)[id].last);
      }, 0);

      const fiatMultiplier = 1 / this.prices(this.baseId)[this.fiatId].last;
